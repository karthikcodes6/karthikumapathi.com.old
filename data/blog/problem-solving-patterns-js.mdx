...
draft: false
summary: Discussing some problem solving patterns
---
...
function same(arr1, arr2) {
  if (arr1.length !== arr2.length) {
    // If array 1 is not equal to array 2 return false
    return false
  }
...
function sumZero(arr) {
  ...
  while (left < right) {
    // Iterate till the two pointer become same
    let sum = arr[left] + arr[right]
    if (sum === 0) {
      // If sum zero found the values return it.
      return [arr[left], arr[right]]
    } else if (sum > 0) {
      // Means the right pointer value will not match anymore so we need to decrease our array order.
      right--
    } else {
      // the left pointer value not be found in right pointer value
      left++
    }
  }
}
